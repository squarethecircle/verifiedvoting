...
def challenge(elements):
    ...
    elem_len = ["%s||%s" % (len(x) , x) for x in elem_str]
    state = "|".join(elem_len)
    H = sha256()
...
class ZKProof(object):
    ...
    def build_proof(self, env, message=""):
        ...
        witnesses = dict(list(env.items()))
        for w in list(self.Sec.keys()):
    ...
    def verify_proof(self, env, sig, message="", strict=True):
        ...
        env_l = [(k,v) for  k,v in list(env.items()) if k in self.Const]
        
        if __debug__ and strict:
            env_not = [k for k,v in list(env.items()) if k not in self.Const]
            if len(env_not):
...
def test_basic():
    ...
    print((list(zk.Const.keys())))
    print((list(zk.Pub.keys())))
    print((list(zk.Sec.keys())))
